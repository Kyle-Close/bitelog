...
export const getUserFoodList = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      let foodInstances: Model<any, any>[] = [];
      if (!includeIngredients) {
        foodInstances = await UserFoods.findAll({
...
export const getManyFoodIngredients = [
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const ingredientIdLists = await getManyIngredientListsByFoodIds(
      req.body.foodIds
    );
    const promises = ingredientIdLists.map((foodIngredientIds) =>
      getIngredientInstancesByIds(foodIngredientIds)
    );
